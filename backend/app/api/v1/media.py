...
async def search_media(
    ...
    try:
        results = await service.search(payload.tg_user_id, payload.query, payload.filter_type)
...
async def add_media_to_library(
    ...
    service = MediaService(session)
    try:
        return await service.add_to_library(payload.tg_user_id, payload.tmdb_id, payload.media_type)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
    except TMDBServiceError as exc:
        raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail="TMDB is unavailable right now.") from exc
