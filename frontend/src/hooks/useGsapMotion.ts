...
export function useGsapAmbientBackground(_scope: RefObject<HTMLElement>, _dependencies: unknown[] = []) {
  // Performance: the ambient layers are large blurred / mix-blend-mode elements
  // rendered behind every glass card. Continuously tweening them forced the GPU
  // to re-rasterize each card's backdrop-filter on every frame (a constant
  // full-screen load => overheating and stutter on mobile). The layers stay
  // visible but static now, so backdrop-filter results can be cached. No
  // perpetual ambient tween is started.
}
